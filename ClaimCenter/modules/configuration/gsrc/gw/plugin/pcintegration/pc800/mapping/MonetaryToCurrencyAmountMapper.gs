...
class MonetaryToCurrencyAmountMapper implements IFieldMapper {
  ...
  override function mapField(converter : ObjectConverter,
    ...
    if(not MonetaryAmount.Type.isAssignableFrom(sourceProperty.FeatureType)) {
    ...
    if(not gw.api.financials.CurrencyAmount.Type.isAssignableFrom(targetProperty.FeatureType)) {
  ...
  private function getMonetaryAmtFromSrc(source:Object, propertyName:String) : MonetaryAmount {
    ...
    if(prop!=null and MonetaryAmount.Type.isAssignableFrom(prop.FeatureType)) {
