...
class ServiceRequestQuoteAndServiceStateHandler extends ServiceRequestStateHandler {
  ...
  private final var SERVICE_REQUEST_OPERATION_DEFINITIONS : Map<ServiceRequestOperation, ServiceRequestOperationDefinition> = {
    ServiceRequestOperation.TC_SUBMITINSTRUCTION -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> { return displaykey.Web.ServiceRequest.StateTransition.Description.InstructionSubmitted}},
    ...
    ServiceRequestOperation.TC_SPECIALISTACCEPTEDWORK -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> {
                                                        if (sr.QuoteStatus == ServiceRequestQuoteStatus.TC_WAITINGFORQUOTE) {
                                                          return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistAcceptedWork.Quote(StringUtil.formatDate(context.ExpectedCompletionDate,"short"))  
                                                        } else {
                                                          return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistAcceptedWork.Service(StringUtil.formatDate(context.ExpectedCompletionDate,"short"))  
                                                        }
                                                      }},
          ...
          :DescriptionExpression    = \sr, context -> { var quote = context.Statement as ServiceRequestQuote
                                                        if (sr.Quotes.Count == 1) { // If it is first quote, record a change for quote added. Otherwise, record a change for quote revised
                                                          return quote.ReferenceNumber.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.AddedQuoteWithReferenceNumber(quote.ReferenceNumber, quote.Amount) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.AddedQuote(quote.Amount)
                                                        } 
                                                        return quote.ReferenceNumber.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.RevisedQuoteWithReferenceNumber(quote.ReferenceNumber, quote.Amount) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.RevisedQuote(quote.Amount)
                                                      }},
    ...
    ServiceRequestOperation.TC_APPROVEQUOTE -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> { var quote = context.Statement as ServiceRequestQuote
                                                        return quote.ReferenceNumber.HasContent ? 
                                                          displaykey.Web.ServiceRequest.StateTransition.Description.QuoteApprovedWithReferenceNumber(quote.ReferenceNumber, quote.Amount) :
                                                          displaykey.Web.ServiceRequest.StateTransition.Description.QuoteApproved(quote.Amount)
                                                      }},                                              
    ...
    ServiceRequestOperation.TC_SPECIALISTCOMPLETEDWORK -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> {return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistCompletedWork}},
    ...
    ServiceRequestOperation.TC_REQUESTREQUOTE -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> { var reason = context.Reason
                                                        return reason.HasContent ? 
                                                          displaykey.Web.ServiceRequest.StateTransition.Description.RequoteRequestedWithReason(reason) :
                                                          displaykey.Web.ServiceRequest.StateTransition.Description.RequoteRequested 
                                                      }},                                            
    ...
    ServiceRequestOperation.TC_SPECIALISTWAITING -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression      = \sr, context -> { var reason = context.Reason
                                                          return reason.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWaitingWithReason(reason) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWaiting 
                                                        }},
    ...
    ServiceRequestOperation.TC_SPECIALISTRESUMEDWORK -> new ServiceRequestOperationDefinition(){
          ...
          :AvailableToSpecialistExpression = \sr, context -> true,
          :NewProgressExpression    = \sr, context -> ServiceRequestProgress.TC_INPROGRESS,
          :DescriptionExpression    = \sr, context -> { return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistResumedWork}},
    ...
    ServiceRequestOperation.TC_UPDATEQUOTEECD -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> { var changeReason = context.Reason
                                                        ...
                                                        return changeReason.HasContent ?
                                                                 displaykey.Web.ServiceRequest.StateTransition.Description.ExpectedCompletionDateChangedWithReason(description, changeReason) :
                                                                 description}},
    ...
    ServiceRequestOperation.TC_SPECIALISTDECLINED -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression      = \sr, context -> { var reason = context.Reason
                                                          return reason.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistDeclinedWorkWithReason(reason) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistDeclinedWork 
                                                        }},
    ...
    ServiceRequestOperation.TC_CANCELSERVICEREQUEST -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression      = \sr, context -> { var reason = context.Reason
                                                          return reason.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.ServiceRequestCanceledWithReason(reason) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.ServiceRequestCanceled
                                                        }},
    ...
    ServiceRequestOperation.TC_SPECIALISTCANCELED -> new ServiceRequestOperationDefinition(){
          ...
          :DescriptionExpression    = \sr, context -> { var reason = context.Reason
                                                          return reason.HasContent ? 
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistCanceledWorkWithReason(reason) :
                                                            displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistCanceledWork
                                                        }}
  }  
