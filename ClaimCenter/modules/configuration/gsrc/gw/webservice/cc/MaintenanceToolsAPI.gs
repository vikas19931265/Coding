...
class MaintenanceToolsAPI {
  ...
  public function getValidBatchProcessNames() : String[] {
    return Delegate.ValidBatchProcessNames as String[];
  }
  ...
  function changeSubtype(publicID : String, targetType : String) {
    var query = gw.api.database.Query.make(Contact).select().where( \ c -> c.PublicID == publicID).first()
    ...
    var claimContact = gw.api.database.Query.make(ClaimContact).select().where(\ cc -> cc.Contact.PublicID == publicID).first()
