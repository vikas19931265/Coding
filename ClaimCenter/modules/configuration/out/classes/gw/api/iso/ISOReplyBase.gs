...
class ISOReplyBase {
  ...
  var _reply : xsd.iso.resp.ClaimInvestigationAddRs as readonly Reply
  ...
  construct(xml : String) {
    _xml = xml
    try {
      _reply = xsd.iso.resp.ClaimInvestigationAddRs.parse(xml)
    } catch (e : Throwable ) {
    ...
    _reportable = gw.transaction.Transaction.getCurrent().loadBean(RequestId.ReportableId) as ISOReportable
  }
  ...
  property get Successful() : boolean {
    return ISOConstants.RESPONSE_STATUS_SUCCESS == Reply.MsgStatus.MsgStatusCd
  }
  ...
  function addReplyAsDocument() {
    ...
    if (nameFormat.HasContent) {
      ...
      var html = processXMLWithXLSTStylesheet(_xml, xslFileURL)
  ...
  static var documentBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder()

  //Style the XML from ISO via the xslt template (provide by ISO) and return the resulting html
  function processXMLWithXLSTStylesheet(origXML : String, xslFileURL : String) : String {
    //parse using builder to get DOM representation of the XML file
    var dom = documentBuilder.parse(new InputSource(new StringReader(origXML)))
  ...
  function logReplyMessage() {
    ...
    _isoProperties.MessageLogger.logResponse(messageId as int, _xml)    
  }
