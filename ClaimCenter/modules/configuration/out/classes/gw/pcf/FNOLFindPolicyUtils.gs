...
class FNOLFindPolicyUtils {
  ...
  function selectPolicyRow(policySummary : PolicySummary, location : pcf.api.Wizard) {
    if (policySummary.Status.equals(PolicyStatus.TC_ARCHIVED)) {
      throw new DisplayableException(displaykey.Wizard.NewClaimWizard.SelectedArchivedPolicy)
    }
    ...
    if (changes) {
      ...
      if (outOfRange) { 
        ...
        if (inRangeMatch) {
          ...
          throw new DisplayableException(displaykey.Wizard.NewClaimWizard.LossDateInOtherPolicyPeriod)
        } else if (ps.EffectiveDate == prevEffectiveDate) {
          throw new DisplayableException(displaykey.Wizard.NewClaimWizard.LossDateOutOfRange)
       } else {
          switchPolicyPeriod(ps, wizardLocation)
          throw new DisplayableException(displaykey.Wizard.NewClaimWizard.LossDateOutOfRangeUseOtherPolicyPeriod)   
       }
      } // if out of range
    } // if changes
