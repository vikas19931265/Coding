...
enhancement GWServiceRequestEnhancement : entity.ServiceRequest {
  ...
  function finishSetup() {
    ...
    if (this.Specialist == null) {
      throw new DisplayableException(displaykey.Web.ServiceRequest.MustSelectSpecialist)
    }
    ...
    this.recordChange(displaykey.Web.ServiceRequest.StateTransition.Description.ServiceCreated, null, null, User.util.CurrentUser.Contact)
  }
  ...
  property get TypePrefixedIncidentName() : String {
    ...
    return  incident == null ? displaykey.Web.ServiceRequest.ClaimLevelServiceRequest : incident.TypePrefixedIncidentName
  }
  ...
  property get SpecialistName() : String {
    return this.Specialist == null ? displaykey.Web.ServiceRequest.NoSpecialistAssigned : this.Specialist.DisplayName
  }
  ...
  property get NotesWithPermCheck() : Note [] {
    ...
    return viewable.intersect(this.Notes as java.util.Collection<entity.Note>) as Note[]
  }
  ...
  property get QuoteLabel(): String {
    if (this.LatestQuote == null) {
      return displaykey.Web.ServiceRequest.Quote.Label
    } else {
      if (this.LatestQuote.DeclinedReason == null) {
        return displaykey.Web.ServiceRequest.Quote.LatestLabel
      } else {
        return displaykey.Web.ServiceRequest.Quote.PriorLabel    
      }
    }
  }
   ...
   */
  function attachDocument(docInfo: DocumentCreationInfo, visibleToSpecialist: boolean) {
  ...
  private function verifyDocumentNotSensitive(document: Document) {
    if (document.SecurityType == DocumentSecurityType.TC_SENSITIVE) {
      throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.SensitiveDocumentException)
    }
  }
  ...
  static function calculateAvailableKinds(specialistServices: SpecialistService[]) : List<ServiceRequestKind> {
    ...
    for (leafService in specialistServices.where(\ s -> s.Leaf)) {
      ...
      if (compatibleKinds.IsEmpty) {
        throw new DisplayableException(displaykey.Web.ServiceRequest.LeafSpecialistServiceWithoutKind(leafService.DisplayName))
      }
  ...
  function createPromotionFromQuoteOnlyServiceRequest(removedServiceTypes: List<ServiceRequestInstructionService>) : ServiceRequest {
    ...
    var newQuoteAndServiceSR = this.Claim.newServiceRequest(this.Instruction.CustomerContact, this.Incident)
    ...
    for (quote in this.Quotes) {
      ...
      newQuote.ServiceRequest = newQuoteAndServiceSR
      newQuote.Description = quote.Description
      newQuote.Amount = quote.Amount
      newQuote.StatementCreationTime = quote.StatementCreationTime
      newQuote.ExpectedDaysToPerformService = quote.ExpectedDaysToPerformService
      newQuote.ReferenceNumber = quote.ReferenceNumber
    ...
    for (prop in CCAssignable.Type.EntityProperties) {
      if(not prop.Autogenerated){
        prop.Accessor.setValue(newQuoteAndServiceSR, prop.Accessor.getValue(this))   
      }
    }
  ...
  function finishSetupForPromotedServiceRequest() {
    ...
    this.recordChange(displaykey.Web.ServiceRequest.StateTransition.Description.ServiceRequestPromotedFrom(this.OriginatingServiceRequest.ServiceRequestNumber), null, null, User.util.CurrentUser.Contact)
    this.OriginatingServiceRequest.recordChange(displaykey.Web.ServiceRequest.StateTransition.Description.ServiceRequestPromoted, null, null, User.util.CurrentUser.Contact)
  }
  ...
  function calculateDelayDescription(originalDate: Date, isServiceDate: boolean) : String {
    ...
    var dateType = isServiceDate ? displaykey.Web.ServiceRequest.ExpectedCompletionDate.Service : displaykey.Web.ServiceRequest.ExpectedCompletionDate.Quote
    var delayValue = displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.NoChange(dateType)
    
    if (originalDate == null) {
      if (isServiceDate and this.ExpectedServiceCompletionDate != null) {
        return displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.ServiceCompletionDateUpdated(StringUtil.formatDate(this.ExpectedServiceCompletionDate, "short"))
      } else if (not isServiceDate and this.ExpectedQuoteCompletionDate != null) {
        return displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.QuoteCompletionDateUpdated(StringUtil.formatDate(this.ExpectedQuoteCompletionDate, "short")) 
      }
      return delayValue
    }
    ...
    if (numberChangedDays == 1) {
      delayValue = displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.DelayedOneDay(dateType, newDate)
    } else if (numberChangedDays > 1) {
      delayValue = displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.Delayed(dateType, numberChangedDays, newDate)
    } else if (numberChangedDays == -1){
      delayValue = displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.BroughtForwardOneDay(dateType, newDate)
    } else if (numberChangedDays < -1){
      delayValue = displaykey.Web.ServiceRequest.StateTransition.ExpectedCompletionDate.BroughtForward(dateType, java.lang.Math.abs(numberChangedDays), newDate)
    } 
    return delayValue
  }
  ...
  private function validateNotAlreadyLinked(document: Document) {
    ...
    if (this.getMatchingServiceRequestDocumentLink(document) != null) {
      throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.DocumentAlreadyLinked(document.Name, this.ServiceRequestNumber))
    }
  }
