...
enhancement GWPaymentEnhancement : Payment
{
  ...
  public function unlinkDeductible() {
    if (!hasDeductibleLineItem()) {
      throw new DisplayableException(displaykey.Deductible.Error.NoDeductibleToUnlinkFromPayment)
    }
  ...
  function removeDeductibleLineItem(tli : TransactionLineItem) {
    if (!tli.isDeductibleLineItem()) {
      throw new DisplayableException(displaykey.Deductible.Error.NonDeductibleTransactionLineItem)
    }
    tli.unlinkDeductible()
    this.removeFromLineItemsIfEditable(tli)
  }
  ...
  internal function shouldTxLineItemBeLinkedToDeductible(tli : TransactionLineItem) : boolean {
    ...
    var tliAmountInCoverageCurrency = tli.getAmountMatchingCurrency(SharedDeductible.CoverageCurrency)

    return (tliAmountInCoverageCurrency != null) and SharedDeductible.Amount == tliAmountInCoverageCurrency.negate()
  }
  ...
  function getCheckWizardPaymentValidationExpression() : String {
    ...
    if(deductibleLineItems.HasElements) {
      ...
      if (deductibleLineItemsTxAmountSum > otherLineItemsTxAmountSum) {
