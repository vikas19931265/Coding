      ...
      try {
        ...
        for (claim in catClaims) {
          ...
          if (catClaims.Count > MAX_CLAIM_RESULTS and claimsProcessedCount >= MAX_CLAIM_RESULTS) {
            PLLoggerCategory.SERVER_BATCHPROCESS.info(displaykey.Web.InternalTools.BatchProcess.CatastropheClaimFinder.ExceedsNumClaimResults(MAX_CLAIM_RESULTS, claimsProcessedCount, catClaims.Count))
            break
          }
        }
