...
class CoverageValidator 
{
    ...
    getExposuresForCoverage(coverage).each(\ e -> {
      if(e.Closed) {
        if(hasTransactions(e)) {
          messages.add(UIMessage.error(displaykey.PolicyRefresh.DiffDisplay.Exposure.HasTransactions(coverage,e)))
        }
      } else {
        messages.add(UIMessage.error(displaykey.PolicyRefresh.DiffDisplay.Coverage.HasOpenExposures(coverage)))
      }
    })
    return messages
  }
  ...
  private function getMessageForCoverageChange(coverage:Coverage, propertyName:String) : String {
    ...
    return displaykey.PolicyRefresh.DiffDisplay.Coverage.FieldChange(PropertyDisplay.getLabel(p), coverage)
  }
