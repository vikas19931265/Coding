    ...
    return new ServiceRequestOperationDefinition() {
        ...
        :ActionExpression           = \sr, context -> {/* It is required that the invoice in the context is added to the Service Request before performing this operation */
          ...
          if (context.PerformedBySpecialist) {
            sr.createNewActivity(ServiceRequestActivityPattern.VENDOR_ADDED_INVOICE, invoice)
          }
        },
        :DescriptionExpression      = \sr, context -> { var invoice = context.Statement as ServiceRequestInvoice
          return invoice.ReferenceNumber.HasContent ?
              displaykey.Web.ServiceRequest.StateTransition.Description.AddedInvoiceWithReferenceNumber(invoice.ReferenceNumber, invoice.Amount) :
              displaykey.Web.ServiceRequest.StateTransition.Description.AddedInvoice(invoice.Amount)
        }}
  }
      ...
      return {
          ServiceRequestOperation.TC_APPROVEINVOICE -> new ServiceRequestInvoiceOperationDefinition() {
              ...
              : DescriptionExpression = \invoice, context -> {
                if (invoice.ReferenceNumber.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceApprovedWithReferenceNumber(invoice.ReferenceNumber, invoice.Amount)
                }
                return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceApproved(invoice.Amount)
              }},
          ServiceRequestOperation.TC_REJECTINVOICE -> new ServiceRequestInvoiceOperationDefinition() {
              ...
              : DescriptionExpression = \invoice, context -> {
                ...
                if (invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceRejectedWithReferenceNumberAndReason(invoice.ReferenceNumber, reason)
                } else if (invoice.ReferenceNumber.HasContent and not reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceRejectedWithReferenceNumber(invoice.ReferenceNumber)
                } else if (not invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceRejectedWithReason(reason)
                }
                return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceRejected
              }},
          ServiceRequestOperation.TC_UNPAYINVOICE -> new ServiceRequestInvoiceOperationDefinition() {
              ...
              : DescriptionExpression = \invoice, context -> {
                var reason = context.Reason
                if (invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceMarkedAsUnpaidWithReferenceNumberAndReason(invoice.ReferenceNumber, reason)
                } else if (invoice.ReferenceNumber.HasContent and not reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceMarkedAsUnpaidWithReferenceNumber(invoice.ReferenceNumber)
                } else if (not invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceMarkedAsUnpaidWithReason(reason)
                }
                return displaykey.Web.ServiceRequest.StateTransition.Description.InvoiceMarkedAsUnpaid
              }},
          ServiceRequestOperation.TC_PAYINVOICE -> new ServiceRequestInvoiceOperationDefinition() {
              ...
              : DescriptionExpression = \invoice, context -> {
                if (invoice.ReferenceNumber.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.InvoicePaidWithReferenceNumber(invoice.ReferenceNumber, invoice.Amount)
                }
                return displaykey.Web.ServiceRequest.StateTransition.Description.InvoicePaid(invoice.Amount)
              }},
          ServiceRequestOperation.TC_WITHDRAWINVOICE -> new ServiceRequestInvoiceOperationDefinition() {
              ...
              : DescriptionExpression = \invoice, context -> {
                var reason = context.Reason
                if (invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWithdrewInvoiceWithReferenceNumberAndReason(invoice.ReferenceNumber, reason)
                } else if (invoice.ReferenceNumber.HasContent and not reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWithdrewInvoiceWithReferenceNumber(invoice.ReferenceNumber)
                } else if (not invoice.ReferenceNumber.HasContent and reason.HasContent) {
                  return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWithdrewInvoiceWithReason(reason)
                }
                return displaykey.Web.ServiceRequest.StateTransition.Description.SpecialistWithdrewInvoice
              }}
      }
    }
