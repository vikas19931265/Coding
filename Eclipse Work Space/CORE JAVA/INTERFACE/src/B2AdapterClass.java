...
If we inherit the adapter class then we dont have to provide implementation for all the 
...
Lets take an example of a class  A that want to implement only one method of an interface B. 
...
Now lets take for example if we want to implement only one method of the interface then we have 
...
Java adapter classes provides the default implementation for listener interface. If we inherit the java adapter
class for listener interface then we dont have to provide complete implementation for listener interface.
...
Lets take for example we have a class A which needs to implement only one method present in the interface. 
If we try to implement the interface then we have to provide implementation for all the methods. Instead
of doing this we can extend the adapter class having implementation for most of the interface methods 
and the ones which are missing can be provided by us in our class saving code and time.
