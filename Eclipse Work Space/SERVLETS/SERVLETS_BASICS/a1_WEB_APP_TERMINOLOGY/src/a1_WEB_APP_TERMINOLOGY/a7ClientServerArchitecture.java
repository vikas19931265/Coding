...
c. To send request and to set response from server we need to use a tool at client machine called as
...
Login--->Location for login servlet. This will be the physical/logica name mapped to the servlet.
...
b.  Where Connectionless Protocol is protocol, it should not require a physical connection, but require a
...
7.  If we provide request parameters along with POST request then GET request will not display the
...
3. EJB Container -- To execute EJB
...
7.  Container is responsible to execute server side applications. Here main server is not aware how to execute 
   ...
   As per the containers physical existency there are 3 types of containers.
