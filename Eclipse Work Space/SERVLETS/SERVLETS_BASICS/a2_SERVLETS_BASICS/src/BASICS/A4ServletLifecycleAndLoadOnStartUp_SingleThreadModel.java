...
4. After the completion of the servlet loading the next task will be as below.
...
2. As per the requirement if we want to make the container to perform servlet loading,
    servlet instantiation and servlet initialization at the time of server start up 
...
3. For some servlets we have to provide such configuration  so that container perform 
...
5. So if we want to load the servlet at the time of the server startup only then we can configure it in the we
...
Static {
	...
	System.out.println(â€œTestServlet2 Loadingâ€�);
}

public TestServlet2{
	
	System.out.println(â€œTestServlet2 instantiationâ€�);
...
public void init(ServletConfig config)
{
	...
	System.out.println(â€œTestServlt2 Initializationâ€�);
}
