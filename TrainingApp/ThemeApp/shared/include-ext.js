...
(function() {
    ...
    var scriptEls = document.getElementsByTagName('script'),
        ...
        theme = getQueryParam('theme') || 'titanium',
        includeCSS = !hasOption('nocss', path),
        neptune = (theme === 'neptune' || theme === 'titanium'),
    ...
    packageName =   theme === 'titanium' ? 'titanium' : 
                    theme === 'classic' ?  'ext-theme-classic' : 'ext-theme' + suffix;

    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + path + '/packages/' + packageName + '/build/resources/' + packageName + '-all-debug.css"/>');
    }
    document.write('<script type="text/javascript" src="' + path + '/ext/ext-all' + (rtl ? '-rtl' : '') + '-dev.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? path + '/..' : path) +
            '/packages/'+ packageName +'/build/' + packageName +
            '-dev.js';
