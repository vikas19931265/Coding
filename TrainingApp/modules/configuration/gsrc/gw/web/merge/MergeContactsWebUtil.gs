...
class MergeContactsWebUtil {
  ...
  private function addHistoryEvent(isKept : boolean, contact : ABContact, name : String, linkId : String) {
    ...
    var history = contact.addHistory(type, displaykey.Web.Contacts.MergeContacts.ContactMerged)
  ...
  private function maybeSetReplacingAddressLinkId(address : Address,
    ...
    if (not replacingAddressData.Included and not replacingAddressData.Primary) {
      var contactKey = isKeptContact ? displaykey.Error.Merge.KeptContact : displaykey.Error.Merge.RetiredContact
      var errorMessage = displaykey.Error.Merge.DuplicateAddressNeedsToBeIncluded(address, addressData.Contact, contactKey,
          displaykey.Web.Contacts.ReviewDuplicateContacts.None)
