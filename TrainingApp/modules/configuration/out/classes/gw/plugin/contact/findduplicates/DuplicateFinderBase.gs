...
abstract class DuplicateFinderBase<T extends ABContact> {
  ...
  abstract function validateMandatoryFields()
  ...
  protected function throwException(contact : ABContact) {
    throw new TooLooseContactDuplicateMatchCriteriaException(contact.Subtype, contact.PrimaryAddress.Country)
  }
   ...
   */
  abstract function makeQueries() : List<Query<T>>

  /**
   * Given that resultABContact is at least a potential match, returns whether it's an exact match.
   */
  abstract function isExactMatch(searchContact : T, resultABContact : T) : boolean
