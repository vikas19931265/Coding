 ...
 */
public interface IDataObjectAPI extends java.rmi.Remote {

    /**
     * <p>
     * Find a set of objects of the same type using the Gosu finder notation.
     * See the <i>Gosu Reference Guide</i> for details on how to build a
     * <b>where</b> clause.
     * The result set's size is capped by default at 100 objects. To override
     * the default, set the <code>GW_MAX_OBJECT_RETRIEVAL_PROPERTY</code>
     * property in the SOAP
     * header using <code>SOAPOutboundHandler</code>.
     * </p>
     * <p>
     * <b>This method is deprecated</b>. Returning all the matching object
     * data without first knowing the
     * size of the matching set can be expensive. Instead, use <code>findIds</code>
     * to find the matching set of ids and
     * <code>getObjectById</code> or <code>getPathValues</code> to load the
...
to just the desired fields should be used if possible.
     ...
     * @throws com.guidewire.ab.webservices.fault.BadIdentifierException If the entity type is invalid
     ...
     * notation. See the <i>Gosu Reference Guide</i> for details on how to
     * build a
     ...
     * </p>
     * <h4>Example</H4>
     * In the following example,  a Gosu finder expression is translated
     * into a call to <code>findIds()</code>:
     * The following Gosu expression:
     * </P>
     * <pre>
     * <code>findIds(Claim with c in Claim where c.LossType == "AUTO" and
     * c.AccidentType != "46")</code>
     * </pre>
     * <p>Becomes the following with IDataObjectAPI call:</p>
     * <pre>
     * <code>findIds("Claim", "c", "c.LossType == \"AUTO\" and c.AccidentType
     * != \"46\"")</code>
     * </pre>
     * <p>
...
clause.
     ...
     * </p><p>
     * If this is a new sequence key a new sequence is created with initial
     * value
     * <i>minValue</i>. If the sequence exists then the next number in the
     * sequence is
...
publicId does not exist.
    ...
    /**
     * <p>
     * Returns properties accessed from a root entity. Specify the paths
     * as
     * strings. A single path may return multiple values if the path traverses
     * an
     ...
     * accessible properties. The set of available properties is a super-set
     * of
     ...
     * </code></pre>
     * <p>
     * Get the public Ids of all the users in a group with a LoadFactor >
     * 50
     * </p>
     * <pre><code>
     * String[] path = new String[1]
...
then its array is empty.
    ...
    /**
     * <p>
     * Add, update, and remove values on entities relative to a root entity.
     * All
     * changes are made in the same transaction. If one change fails none
     * of
     * the changes are applied.
     * </p><p>
     * A path can traverse any entity property but only a writeable property's
     * value can be set. An entity's writeable properties are the same set
     * of
     * properties accessible from Gosu. See the API Help in Studio for a
     * list of
     * accessible properties. The set of accessible properties is a super-set
     * of
     ...
     * </p><p>
     * Specify the path in XPath notation. The notation draws heavily from
     * the <a href="http://xml.coverpages.org/IBM-BEA-SDOv10.pdf">"Service
     * Data Object"</a>
     ...
     * <H4>Add Example</H4>
     * <p>
     * This example associates an existing user to a group by adding a new
     * <code>GroupUser</code> entity
     * linking the two. The value of <code>newGroupUser/user</code> on the
     * <code>newGroupUser</code> is
     * in the form <i>entityType:publicId</i>, where <code>user</code> is
     * the <i>entityType</i> and
     ...
     * <p>
     * Disassociate a user from a group by removing the <code>GroupUser</code>
     * entity that
     * links them. <code>74</code> is <i>publicId</i> of the user being removed
     * from the group.
     * </p>
     * <pre><code>
...
key link. An empty path ("") will remove the root entity.
    ...
    /**
     * <p>
     * Update values on entities relative to a root entity. All changes will
     * be
     * made in the same transaction. If one change fails none of the changes
     * will
     * be applied.
     * </p><p>
     * A path can traverse any entity property but only a writeable property's
     ...
     * the "exportable" properties that exist on SOAP entities.
     * </p><p>
     * The leaf of a path must reference a "simple" property (not an entity
     * or
     * array) but the path can traverse through entities and arrays.
     * </p><p>
     * Specify the path in XPath notation. The notation draws heavily from
