...
class Snapshot
{
  ...
  public static function getCoverageName(claim : dynamic.Dynamic, coverage : dynamic.Dynamic) : String {
    ...
    if (coverage == null) {
      return result
    } else if (getEntityType(coverage) == "PropertyCoverage") {
      for (policyProperty in claim["Policy"]["Properties"]) {
        if (policyProperty["Property"]["Coverages"].hasMatch(\ propertyCoverage -> propertyCoverage == coverage)) {
          result = getDisplayName(policyProperty)
          break
        }
      }
    } else if (getEntityType(coverage) == "VehicleCoverage") {
      for (policyVehicle in claim["Policy"]["Vehicles"]) {
        if (policyVehicle["Vehicle"]["Coverages"].hasMatch(\ vehicleCoverage -> vehicleCoverage == coverage)) {
  ...
  public static function getDisplayName( bean : dynamic.Dynamic ) : String
  {
    ...
    switch( strTypeName )
    {
      ...
      case "Exposure":
      {
        var retString = "(" + bean["ClaimOrder"] + ") " + bean["ExposureType"] + " - "
