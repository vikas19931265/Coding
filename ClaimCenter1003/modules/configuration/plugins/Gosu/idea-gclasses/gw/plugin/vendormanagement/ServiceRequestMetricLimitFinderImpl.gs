...
class ServiceRequestMetricLimitFinderImpl implements IServiceRequestMetricLimitFinderPlugin {
  ...
  override function findLimitValues(metric : ServiceRequestMetric) : ServiceRequestMetricLimitValues {
    ...
    if (not limitsForType.Empty) {
      ...
      for (service in metric.ServiceRequest.Instruction.OrderedServices) {
        var serviceAncestry = service.Service.Ancestry
        var serviceRequest = metric.ServiceRequest
        serviceAncestry.each(\ancestry -> {
                var map = limitsForType.partition(\lim -> {
                  return qualityOfLimitMatch(lim, serviceRequest.Tier, serviceRequest.Claim.Policy.CustomerServiceTier, serviceAncestry.first(), serviceAncestry.last(), serviceRequest.Currency)
                })
