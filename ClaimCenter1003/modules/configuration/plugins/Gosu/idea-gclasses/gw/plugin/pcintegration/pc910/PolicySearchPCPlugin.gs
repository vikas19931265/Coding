package gw.plugin.pcintegration.pc910

uses gw.api.locale.DisplayKey
uses gw.api.util.DisplayableException
uses gw.plugin.InitializablePlugin
uses gw.plugin.policy.search.IPolicySearchAdapter
uses wsi.remote.gw.webservice.pc.pc910.ccpolicysearchintegration.CCPolicySearchIntegration
uses wsi.remote.gw.webservice.pc.pc910.entities.types.complex.CCPCFilteringCriteria
...
class PolicySearchPCPlugin implements IPolicySearchAdapter, InitializablePlugin {
  ...
  override function searchPolicies( criteria : PolicySearchCriteria ) : PolicySearchResultSet   {
    ...
    var pcCriteria = gw.plugin.pcintegration.pc910.PolicySearchConverter.INSTANCE.createPCSearchCriteria( criteria )

    // Just create an empty filter by default
    var pcFilter = new CCPCFilteringCriteria()

    var pcSummaries = PolicySearchService.searchForPolicies( pcCriteria, pcFilter )
    var ccSummaries = gw.plugin.pcintegration.pc910.PolicySearchConverter.INSTANCE.convertPCPolicySummary( pcSummaries )
  ...
  private function retrievePolicy( policyNumber : String, lossDate: Date, pcFilter : CCPCFilteringCriteria ) : PolicyRetrievalResultSet   {
    ...
    try {
      ...
      resultSet.Result = gw.plugin.pcintegration.pc910.PolicySearchConverter.INSTANCE.convertPCPolicy( env.CCPolicy.$TypeInstance )
    }
    catch( e : Throwable ) {
