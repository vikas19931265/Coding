...
enhancement GWClaimWorkCompEnhancement: entity.Claim {
   ...
   */
  public function getWCContactEmployerActivityStatus() : Triple<String, GWColor, String> {
   ...
   * <p>@return Pair which contains the icon filename (first) and tooltip (second)</p>
   */
  public function getWCContactEmployeeActivityStatus() : Triple<String, GWColor, String> {
    return this.LossType == LossType.TC_WC
   ...
   * <p>@return Pair which contains the icon filename (first) and tooltip (second)</p>
   */
  public function getWCContactMedProviderActivityStatus() : Triple<String, GWColor, String> {
    return this.LossType == LossType.TC_WC
        ? getWCThreePointContactActivityStatus(MedProviderContactActivityPatternCode)
        : null
  }

  private function getWCThreePointContactActivityStatus(final code: String) : Triple<String, GWColor, String> {
    ...
    if (activity.Status == ActivityStatus.TC_OPEN) {
      ...
      iconColor = GWColor.THEME_OUTCOME_POSITIVE
      tooltip = DisplayKey.get("Web.ClaimSummary.Headline.ThreePointContact.Completed", contactTypeMap.get(code))
    }

    return new Triple<String, GWColor, String>(icon, iconColor, tooltip)
  }
