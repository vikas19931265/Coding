...
enhancement GWFixedPropertyIncidentEnhancement : entity.FixedPropertyIncident {
  ...
  property get PropertyAddressValueRange() : PolicyLocation[] {
    ...
    var thirdParty : Set<PolicyType> = {
        LOBAbstraction.ForPolicy.PersonalAutoPolicyType,
        LOBAbstraction.ForPolicy.CommercialAutoPolicyType,
        LOBAbstraction.ForPolicy.PersonalTravelPolicyType,
        LOBAbstraction.ForPolicy.HOPPolicyType
    }
  ...
  /**
   * Get the integer format for number of items.
   * @return the integer format that is used to format the number of items
   */
  property get IntegerFormat() : String {
    return "###,###,###"
  }
