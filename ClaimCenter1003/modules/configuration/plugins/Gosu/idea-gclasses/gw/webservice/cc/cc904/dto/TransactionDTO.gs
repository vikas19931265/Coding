 ...
 * A {@link TransactionDTO} is a DTO that represents an instance of {@link entity.Transaction} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link entity.Transaction}.
...
final class TransactionDTO {
  ...
  final function readFrom(that: Transaction): TransactionDTO {
    ...
    // if field is on base class
      ...
      Status                        = that.Status
// SubmitDate deleted. If restored in Transaction.etx or you are fine ignoring the value, you can delete this line and perhaps uncomment the next
      if (SubmitDate != null) throw new IllegalArgumentException("SubmitDate has been deleted. Add back to datamodel, or do not set it in the Web Service call.")
//      SubmitDate                    = that.SubmitDate
   ...
   */
  reified function writeTo<T extends Transaction>(that: T, ignoreNullValues = true): T {
    // if field is on base class
...
//      if (SubmitDate                   != null or !ignoreNullValues) that.SubmitDate                   = SubmitDate
   ...
   * @param bundleProvider The entity in whose Bundle to create the new Transaction.
   ...
   */
  reified function writeToNewEntityIn<T extends Transaction>(bundleProvider: BundleProvider, createNew: block(): T = null, ignoreNullValues = true): T {
    var instance: T
