package gw.webservice.cc.cc1000
...
uses gw.webservice.cc.cc1000.dto.AddressDTO
uses gw.webservice.cc.cc1000.dto.BuildingDTO
uses gw.webservice.cc.cc1000.dto.ClaimContactDTO
uses gw.webservice.cc.cc1000.dto.ClaimContactRoleDTO
uses gw.webservice.cc.cc1000.dto.ClaimDTO
uses gw.webservice.cc.cc1000.dto.ContactDTO
uses gw.webservice.cc.cc1000.dto.CoverageDTO
uses gw.webservice.cc.cc1000.dto.DocumentDTO
uses gw.webservice.cc.cc1000.dto.ExposureDTO
uses gw.webservice.cc.cc1000.dto.IncidentDTO
uses gw.webservice.cc.cc1000.dto.NoteDTO
uses gw.webservice.cc.cc1000.dto.PolicyDTO
uses gw.webservice.cc.cc1000.dto.PolicyLocationDTO
uses gw.webservice.cc.cc1000.dto.PolicySummaryDTO
uses gw.webservice.cc.cc1000.dto.RiskUnitDTO
uses gw.webservice.cc.cc1000.dto.VehicleDTO
 ...
 */
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc1000/CCWsiEntityApiBase")
...
abstract class CCWsiEntityApiBase {
  ...
  protected function assembleFromDTO(dto : ClaimDTO, policy : Policy, bundleProvider : BundleProvider) : Claim {
    ...
    assembleFromDTOs(dto.NewContacts, bundleProvider, \ ctc -> {
      ctc.Claim = claim
    })
    assembleFromDTOs(dto.NewDocuments, bundleProvider, \ doc -> {
      doc.Claim = claim
    })
    assembleFromDTOs(dto.NewIncidents, bundleProvider, \ inc -> {
      inc.Claim = claim
    })
    assembleFromDTOs(dto.NewNotes, bundleProvider, \ nte -> {
      nte.Claim = claim
    })
  ...
  protected function assembleFromDTO(dto : PolicyDTO, bundleProvider : BundleProvider) : Policy {
    ...
    assembleFromDTOs(dto.NewCoverages, bundleProvider, \ cov -> {
      cov.Policy = policy
    })
    assembleFromDTOs(dto.NewPolicyLocations, bundleProvider, \ loc -> {
      loc.Policy = policy
    })
    assembleFromDTOs(dto.NewVehicles, bundleProvider, \ vru -> {
      vru.Policy = policy
    })
  ...
  protected function assembleFromDTOs(dtos : ActivityDTO[], claim : Claim, exposure : Exposure, setup(act : Activity)) : Activity[] {
    return dtos.map(\ dto -> {
      ...
      assembleFromDTOs(dto.NewNotes, claim, \ note -> {
  ...
  protected function assembleFromDTOs(dtos : ExposureDTO[], claim : Claim, constructIncident : (exp : Exposure) : Incident, setup(exp : Exposure)) : Exposure[] {
    return dtos.map(\ dto -> {
      ...
      assembleFromDTOs(dto.NewDocuments, claim, \ doc -> {
  ...
  protected function assembleFromDTOs(dtos : PolicyLocationDTO[], bundleProvider : BundleProvider, setup(loc : PolicyLocation)) : PolicyLocation[] {
    return dtos.map(\ dto -> {
      ...
      assembleFromDTOs(dto.NewBuildings, bundleProvider, \ building -> {
        policyLocation.addToBuildings(building)
      })
      assembleFromDTOs(dto.NewLocationBasedRisks, bundleProvider, \ ru -> {
        (ru as LocationBasedRU).PolicyLocation = policyLocation
      })
    ...
    return dtos.map(\ dto -> {
      ...
      if (riskUnit typeis LocationBasedRU) {
        if (dto.NewBuilding != null) {
          assembleFromDTOs({dto.NewBuilding}, bundleProvider, \ bldg -> {
            riskUnit.Building = bldg
          })
        }
        if (dto.NewPolicyLocation != null) {
          assembleFromDTOs({dto.NewPolicyLocation}, bundleProvider, \ ploc -> {
            riskUnit.PolicyLocation = ploc
          })
        }
      }
      ...
      if (riskUnit typeis VehicleRU) {
        if (dto.NewVehicle != null) {
          assembleFromDTOs({dto.NewVehicle}, bundleProvider, \ vehicle -> {
            riskUnit.Vehicle = vehicle
          })
        }
      }
      ...
      setup(riskUnit)
      assembleFromDTOs(dto.NewCoverages, bundleProvider, \ cov -> {
        (cov as RUCoverage).RiskUnit = riskUnit
      })
      return riskUnit
    })
  }
  ...
  private function createNewExposure(dto : ExposureDTO, claim : Claim, constructIncident : (exp : Exposure) : Incident) : Exposure {
    ...
    exposure.CreatedVia = CreatedVia.TC_WEB_SERVICE
    return exposure
  }
   ...
   */
  protected reified function fetchByPublicID<U extends KeyableBean>(publicID : String, bundle : Bundle) : U {
   ...
   * @param publicID The PublicID of the object to fetch from the database
   */
  protected reified function fetchByPublicID<U extends KeyableBean>(publicID : String) : U {
   ...
   */
  protected reified function fetchByPublicID<U extends KeyableBean>(publicID : String, nullOk : boolean) : U {
   ...
   * @param bundle   The bundle to which to add the fetched object
   */
  protected reified function fetchByPublicID<U extends KeyableBean>(publicIDs : String[]) : Iterator<U> {
