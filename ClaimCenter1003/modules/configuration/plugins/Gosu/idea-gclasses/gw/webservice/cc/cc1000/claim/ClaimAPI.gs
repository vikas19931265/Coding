package gw.webservice.cc.cc1000.claim
...
uses gw.webservice.cc.cc1000.dto.ActivityPatternDTO
uses gw.webservice.cc.cc1000.dto.AddressDTO
uses gw.webservice.cc.cc1000.dto.ClaimDTO
uses gw.webservice.cc.cc1000.dto.ClaimInfoDTO
...
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc1000/claim/ClaimAPI")
...
class ClaimAPI extends CCWsiEntityApiBase {
  ...
  public function createActivity(dto : ActivityDTO) : String {
    ...
    return runTxnWithResult(\bundle -> {
      var claim    = bundle.add(dto.Claim)
      var exposure = bundle.add(dto.Exposure)
      return assembleFromDTOs({dto}, claim, exposure, NOOP)
    }).first().PublicID
  }
  ...
  public function createAddress(dto : AddressDTO) : String {
    ...
    return runTxnWithResult(\bundle -> assembleFromDTO(dto, bundle)).PublicID
  }
  ...
  function addFNOL(claimDTO : ClaimDTO, policyDTO : PolicyDTO) : String {
    ...
    return runTxnWithResult(\bundle -> {
      ...
      var fnolClaim = assembleFromDTO(claimDTO, bundle, policyDTO)
  ...
  function addFNOLLookupPolicy(claimDTO : ClaimDTO, policySummaryDTO : PolicySummaryDTO) : String {
    ...
    return runTxnWithResult(\bundle -> {
      ...
      var fnolClaim = assembleFromDTO(claimDTO, bundle, policySummaryDTO)
  ...
  function importClaimFromXML(xml : String, mapperClassName : String) : String {
    return runTxnWithResult(\bundle -> {
      ...
      return Delegate.importFNOLFromXMLWithoutCommit(xml, fnolMapper, bundle)
    }).PublicID
  }
  ...
  function migrateClaim(claimDTO : ClaimDTO, policyDTO : PolicyDTO) : String {
    return runTxnWithResult(\bundle -> {
      ...
      var claim = assembleFromDTO(claimDTO, bundle, policyDTO)
  ...
  function doesExist(claimID : String) : boolean {
    return fetchByPublicID<Claim>(claimID, true) != null
  }
  ...
  function checkValid(claimID : String, validationLevel : ValidationLevel) : boolean {
    return runTxnWithResult(\bundle -> {
      ...
      return !validationResult.hasErrors(validationLevel)
    })
  }
   ...
   */
  @Deprecated("Use gw.webservice.cc.cc904.claim.ClaimAPI#startClaimValidation(java.lang.String) instead")
  ...
  */
  ...
  function createClaimContact(claimPublicID: String, contactDTO: ContactDTO, role: ContactRole): String {
    return runTxnWithResult(\bundle -> {
      var claim = fetchByPublicID<Claim>(claimPublicID, bundle)
      return assembleFromDTOs(
          {contactDTO}.toTypedArray(),
          claim,
          role
      ).single()
    }).PublicID
  ...
  */
  ...
  public function createDocument(dto : DocumentDTO) : String {
    ...
    return runTxnWithResult(\bundle -> assembleFromDTOs({dto}, bundle, NOOP).single()).PublicID
  }
  ...
  function createExposures(claimID : String, exposures : ExposureDTO[]) : String[] {
    ...
    return runTxnWithResult(\bundle -> {
      var claim = fetchByPublicID<Claim>(claimID, bundle)
      return assembleFromDTOs(exposures, claim, NOOP)
    })*.PublicID
  }
  ...
  function createCustomHistoryWithDesc(claimID : String, historyType : CustomHistoryType, description : String) : String {
    return runTxnWithResult(\bundle -> {
      var claim = fetchByPublicID<Claim>(claimID, bundle)
      return claim.createCustomHistoryEvent(historyType, description)
    }).PublicID
  }
  ...
  public function createNote(dto : NoteDTO) : String {
    ...
    return runTxnWithResult(\bundle ->
        assembleFromDTOs({dto}, bundle, NOOP).single()
    ).PublicID
  }
