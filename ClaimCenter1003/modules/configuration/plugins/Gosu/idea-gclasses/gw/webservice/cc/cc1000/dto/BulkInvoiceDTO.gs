 ...
 * A(n) {@link gw.webservice.cc.cc1000.dto.BulkInvoiceDTO} is a DTO that represents an instance of {@link BulkInvoice} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link BulkInvoice}.
 ...
 * <p>The specific mappings for {@link gw.webservice.cc.cc1000.dto.BulkInvoiceDTO} are as follows:
 ...
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc1000/dto/BulkInvoiceDTO")
final class BulkInvoiceDTO {
  ...
  var _approvedReportingAmount       : CurrencyAmount                   as ApprovedReportingAmount
  var _approvedTransactionAmount     : CurrencyAmount                   as ApprovedTransactionAmount
  ...
  var _bulkInvoiceTotal              : CurrencyAmount                   as BulkInvoiceTotal
  ...
  var _mailingAddressPublicID        : String                           as MailingAddressPublicID
  var _mailTo                        : String                           as MailTo
  var _memo                          : String                           as Memo
  var _newInvoiceItems               : BulkInvoiceItemDTO[]             as NewInvoiceItems = {}
  var _newMailingAddress             : AddressDTO                       as NewMailingAddress
  ...
  var _totalReportingAmount          : CurrencyAmount                   as TotalReportingAmount
  var _totalTransactionAmount        : CurrencyAmount                   as TotalTransactionAmount
  ...
  final function readFrom(that : BulkInvoice) : BulkInvoiceDTO {
    ...
    // if field is on base class
      ...
      MailingAddressPublicID         = that.MailingAddress.PublicID
      MailTo                         = that.MailTo
      Memo                           = that.Memo
  ...
  final function writeTo(that : BulkInvoice, ignoreNullValues = true) : BulkInvoice {
    // if field is on base class
      ...
      if (MailingAddressPublicID        != null or !ignoreNullValues) that.MailingAddress                = MailingAddress
    ...
    //
    if (NewMailingAddress != null) {
      that.MailingAddress = NewMailingAddress.writeToNewEntityIn(that.Bundle)
    }
    return that
  }
   ...
   * @param bundleProvider The entity in whose Bundle to create the new BulkInvoice.
   ...
   */
  final function writeToNewEntityIn(bundleProvider: BundleProvider, createNew: block(): BulkInvoice = null, ignoreNullValues = true): BulkInvoice {
  ...
  /** Convenience property that returns the {@link Address} whose PublicID is {@code MailingAddressPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get MailingAddress()             : Address                  { return fetchByPublicID(MailingAddressPublicID) }
