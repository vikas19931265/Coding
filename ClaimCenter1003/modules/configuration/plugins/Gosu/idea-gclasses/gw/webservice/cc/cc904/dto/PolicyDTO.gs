 ...
 * A {@link PolicyDTO} is a DTO that represents an instance of {@link entity.Policy} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link entity.Policy}.
 ...
 * <table border="1"><tr><th>Field</th><th>Maps to</th></tr><tr><td>CoveragePublicIDs</td><td>Policy.Coverages*.PublicID</td></tr><tr><td>CreateTime</td><td>Policy.CreateTime</td></tr><tr><td>CreateUserPublicID</td><td>Policy.CreateUser.PublicID</td></tr><tr><td>Currency</td><td>Policy.Currency</td></tr><tr><td>EffectiveDate</td><td>Policy.EffectiveDate</td></tr><tr><td>ExpirationDate</td><td>Policy.ExpirationDate</td></tr><tr><td>insuredPublicID</td><td>Policy.insured.PublicID</td></tr><tr><td>PolicyLocationPublicIDs</td><td>Policy.PolicyLocations*.PublicID</td></tr><tr><td>PolicyNumber</td><td>Policy.PolicyNumber</td></tr><tr><td>PolicyType</td><td>Policy.PolicyType</td></tr><tr><td>ProducerCode</td><td>Policy.ProducerCode</td></tr><tr><td>PublicID</td><td>Policy.PublicID</td></tr><tr><td>Status</td><td>Policy.Status</td></tr><tr><td>TotalVehicles</td><td>Policy.TotalVehicles</td></tr><tr><td>UpdateTime</td><td>Policy.UpdateTime</td></tr><tr><td>UpdateUserPublicID</td><td>Policy.UpdateUser.PublicID</td></tr><tr><td>VehiclePublicIDs</td><td>Policy.Vehicles*.PublicID</td></tr><tr><td>Verified</td><td>Policy.Verified</td></tr></table></p>
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc904/dto/PolicyDTO")
final class PolicyDTO {
  /** Derived from Policy#Coverages*.PublicID */
  ...
  /** Derived from Policy#insured.PublicID */
  var _insuredPublicID         : String              as insuredPublicID
  var _newCoverages            : CoverageDTO[]       as NewCoverages = {}
  var _newInsured              : ContactDTO          as NewInsured
  var _newPolicyLocations      : PolicyLocationDTO[] as NewPolicyLocations = {}
  var _newVehicles             : RiskUnitDTO[]       as NewVehicles = {}
  /** Derived from Policy#PolicyLocations*.PublicID */
  var _policyLocationPublicIDs : String[]            as PolicyLocationPublicIDs = {}
  ...
  protected function _copyReadOnlyFieldsFrom(that: Policy) {
    ...
    _policyLocationPublicIDs = that.PolicyLocations*.PublicID
   ...
   * @param bundleProvider The entity in whose Bundle to create the new Policy.
   ...
   */
  final function writeToNewEntityIn(bundleProvider: BundleProvider, createNew: block(): Policy = null, ignoreNullValues = true): Policy {
    var instance: Policy
  ...
  /** Convenience property that returns a {@link PolicyCoverage[]} of the objects whose PublicIDs are in {@code CoveragePublicIDs}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get Coverages()            : PolicyCoverage[]   { return fetchByPublicID(CoveragePublicIDs) }
  ...
  /** Convenience property that returns a {@link PolicyLocation[]} of the objects whose PublicIDs are in {@code PolicyLocationPublicIDs}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get PolicyLocations()      : PolicyLocation[]   { return fetchByPublicID(PolicyLocationPublicIDs) }
