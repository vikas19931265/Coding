...
class ISOClaimSearchRequestBase extends ISORequest {
   ...
   * verifyCanPopulate). If it can then the loss section is added and
  ...
  protected function addExposureToClaimLevelSearchRequest(exposure: Exposure) {
    ...
    if (lossSection != null) {
      var errorMessage = verifyPrePopulate(lossSection)

      if (!errorMessage.HasContent) {
        lossSection.populate()
        errorMessage = verifyPostPopulate(lossSection)
      }
   ...
   * Only used for claim level messaging
   */
  protected function verifyPrePopulate(lossSection: ISOLossSection) : String {
  ...
  protected function verifyPostPopulate(lossSection: ISOLossSection): String {
    return lossSection.verifyPostPopulate(ClaimLevelLossSections)
  }
