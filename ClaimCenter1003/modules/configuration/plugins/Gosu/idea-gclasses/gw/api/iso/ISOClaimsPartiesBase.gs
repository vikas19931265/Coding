...
class ISOClaimsPartiesBase {
  ...
  /**
   * Return the set of claim parties
   */
  property get AllContacts(): Set<Contact> {
    return _partiesByContact.Keys.freeze()
  }
    ...
    if (role != existingRole) {
      if (existingRole.IsClaimant || existingRole.IsInsured) {
        if (existingRole.IsClaimant && existingRole.IsInsured) {
          ...
          setRole(owner, claimsParty, role)
        } // else leave alone; new role is no more specific than the old one, or it's not a claimant/insured role at all
      } else if (role.IsClaimant || role.IsInsured) {
        setRole(owner, claimsParty, role)
      } else if (role.IsIndividual && !existingRole.IsIndividual) {
        setRole(owner, claimsParty, role)
      } // else leave alone, new role is no more specific than the old one
    }
  }
