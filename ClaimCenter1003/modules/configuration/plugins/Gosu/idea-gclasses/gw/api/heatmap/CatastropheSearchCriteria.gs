...
class CatastropheSearchCriteria {
  ...
  var _maxNumberOfBindVariables : int as MaxNumberOfBindVariables = HeatMapGenerator.MaxNumberOfBindVariables
  ...
  function updateMapAndTable() : boolean {  // return value is not meaningful
    if (EnterPage or not HeatMapGenerator.HeatMapConfigured) // filter out duplicate call when entering page, ignore call if map not configured
      return true
  ...
  // Generates a string composed of the search criteria used when calculating the Catastrophe map's view.
  // (Should be kept in sync with the method 'wereSettingsChanged')
  property get SettingsString() : String {
    return String.join(":", {
        Long.toString(HeatMap.CatastropheKey?.Value), // This is the only param not checked in 'wereSettingsChanged'
        HeatMap.ClaimState?.Code,
        HeatMap.ReportedDateRange?.Code,
        HeatMap.AssignedToGroup?.toString(),
        HeatMap.PolicyLocations?.Name,
        HeatMap.MapView.FilterOptionLabel})
  }
  ...
  static function dateFilterChoices() : List<DateRangeChoiceType> {
    if (dateFilterChoices == null) {
      ...
      dateFilterChoices = choices.where(\ choice -> choice.Categories != null and choice.Categories.hasMatch(\ d -> d.Code == "claim"))
    }
