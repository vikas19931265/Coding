...
class CreateRIData extends CCBaseCommand  {
 ...
 private function createOpenPropertyClaim() :Claim { 
  ...
  ExposureBuilder.uiReadyHOPContent().onClaim(claim).withCoverageSubType(TC_BADOCLIABILITYCOVPD).create()
  ExposureBuilder.uiReadyHOPContent().onClaim(claim).withCoverageSubType(TC_BABOBTAILLIABCOV_PD).create()
  claim.Bundle.commit()
  return   claim
 }
 ...
 private function setUpFinancialData(exp1:Exposure, exp2: Exposure, exp3: Exposure) {
   ...
   private function setUpRIData (exp1:Exposure, exp2: Exposure, exp3: Exposure) {
     ...
     var group1 = RIAgreementGroupBuilder.uiReadyHOP()
  ...
  if (exp3 != null){
    RIAgreementGroupBuilder.uiReadyHOP()
