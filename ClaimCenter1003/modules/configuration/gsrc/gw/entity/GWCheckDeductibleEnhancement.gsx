...
enhancement GWCheckDeductibleEnhancement : Check
{
  ...
  public function unlinkDeductibles() {
    PrimaryCheck.Payments*.LineItems
        .where(\tli -> tli.LineCategory == LineCategory.TC_DEDUCTIBLE)
        .each(\tli -> tli.unlink())
  }

  private property get PrimaryCheck(): entity.Check {
    return this.Group == null ? this : this.Group.PrimaryCheck
  }
  ...
  /**
   * Removes all deductible line items from all payments on this check.
   * This is different from unlinkDeductibles(), which only removes the association from deductible line items to
   * deductibles, but leaves the items on the payments.
   */
  public function unapplyDeductibles() {
    this.Payments.each(\payment -> payment.unapplyAndRemoveDeductibleLineItem(payment.SharedDeductible))
  }
}
