...
enhancement GWSubroExposureEnhancement : entity.Exposure {
  ...
  property get DeductiblePaid() : CurrencyAmount {
    ...
    for (deductible in this.Claim.Deductibles) {
      for (tli in deductible.TransactionLineItems) {
        if (tli.LinkedToDeductible and tli.Transaction.Exposure == this and tli.Transaction.CostType == TC_CLAIMCOST) {
