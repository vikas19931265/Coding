...
abstract class ISOLossSectionBase {
  ...
  public function verifyCanPopulate(existingLossSections: List<ISOLossSectionBase>): String {
    return this.matchesAny(existingLossSections)
        ? DisplayKey.get("Java.Error.ISO.WrongLossInfoCoverageAndLossCause")
        : null
  }

  public function verifyPostPopulate(existingLossSections: List<ISOLossSectionBase>): String {
    return null
  }

  function matchesAny(lossSections: List<ISOLossSectionBase>): boolean {
    return lossSections.hasMatch(\lossSection -> this.matches(lossSection))
  }

  function matches(otherLossSection: ISOLossSectionBase): boolean {
    if (otherLossSection != this
        and Claimant == otherLossSection.Claimant
        and LossSectionAggregateType == otherLossSection.LossSectionAggregateType) {
      var thisCodes = CoverageCodes
      var otherCoverageCodes = otherLossSection.CoverageCodes
      return thisCodes.CoverageType == otherCoverageCodes.CoverageType
          and thisCodes.LossType == otherCoverageCodes.LossType
    }
    return false
  }
  ...
  protected function createItemDefinition(vehicle : Vehicle, itemType : String) : ItemDefinition {
    ...
    if (ISOConstants.MOBILE_EQUIPMENT_ITEM_TYPE == itemType) {
