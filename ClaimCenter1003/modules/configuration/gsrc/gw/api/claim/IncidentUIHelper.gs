...
abstract class IncidentUIHelper {
  ...
  /**
   * Removes the incident from the loss details page of a {@link Claim}
   * @param claim the claim to remove the incident from
   * @param incident the {@link Incident}
   * @param CurrentLocation the {@link CurrentLocation} of the page
   * @param wizard the {@link NewClaimWizardInfo} that is used to navigate the FNOL claim
   */
  function removeIncident(claim: Claim, incident: Incident, CurrentLocation: pcf.api.Location, wizard: gw.api.claim.NewClaimWizardInfo) {
    if (wizard != null) {
      // remove incident from the FNOL page
      claim.removeFromIncidents(incident);
      gw.api.claim.FnolServiceRequestHelper.removeIncidentServiceRequests(incident, wizard)
    } else if (!CurrentLocation.InEditMode) {
  ...
  /**
   * Views the incident in a popup
   * @param incident the {@link Incident}
   * @param inEditMode whether or not the claim is currently in edit mode
   * @param wizard the {@link NewClaimWizardInfo} that is used to navigate the FNOL claim
   */
  function viewIncident(incident: Incident, inEditMode: boolean, wizard: gw.api.claim.NewClaimWizardInfo) {
    if (wizard != null) {
      goToViewIncidentInFNOLWizard(incident)
    } else {
      goToViewIncident(incident, inEditMode)
    }
  }
}
