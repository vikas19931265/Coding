 ...
 * A(n) {@link gw.webservice.cc.cc1000.dto.ClaimContactDTO} is a DTO that represents an instance of {@link ClaimContact} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link ClaimContact}.
 ...
 * <p>The specific mappings for {@link gw.webservice.cc.cc1000.dto.ClaimContactDTO} are as follows:
 ...
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc1000/dto/ClaimContactDTO")
final class ClaimContactDTO {
  ...
  var _newContact            : ContactDTO            as NewContact
   ...
   * @param bundleProvider The entity in whose Bundle to create the new ClaimContact.
   ...
   */
  final function writeToNewEntityIn(bundleProvider: BundleProvider, createNew: block(): ClaimContact = null, ignoreNullValues = true): ClaimContact {
  ...
  /** Convenience property that returns the {@link entity.Contact} whose PublicID is {@code ContactPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
