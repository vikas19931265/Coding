...
6. Client side scripts  are the scripts which run on the browser and processes requests without call to the server. example html, css, javascript are client side
   programming languages. Whatever coding we do with these languages its effect will be visible in browser(client) , however this differs with the 
   server side programming like servlets/jsp as the coding and functionality is not visible to the client.There is something which happens at 
   the backend.

7. The web browser downloads the web page from the server and processes the code within the page to render information to the user.

8. In server side scripting, the software runs on the server and processing is completed in the server then plain pages are sent to the user.
