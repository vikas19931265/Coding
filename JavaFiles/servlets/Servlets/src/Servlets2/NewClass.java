...
Browser makes request and reches server

Server identifies for which resouce request is coming

server will execute the component

generates response given to network

network take ressponse and carry it to the client
...
for the distributed applicatoin no browser

At client side and server machine side that is remote machine
java program is present.

Only one JVM needed in web application that is needed at teh server side

in remote based distributed application JVM is required in both the machines
that is client and the server.

Why becaue at local machine java code is present and remote machine also the 
...
5. The main purpose of web application is to generate dynamic response but in starting
  ...
  in startin they were able to manage only static pages. But prestly the purpose is not
    ony the static response but also dynamic response is also generated.
  ...
  Lets take an example of durga software
    ...
    and icici bank willl have its own set of prgramming
    
    Hence this interaction is distributed applicaton
...
----------------------------------web application-----     ---------------distrubuted application------    
