...
2. Container is for execution of the server side components and web server is just like an admin.
...
2. Deploy all the web applications to the server that is storing all the applications into the server
   space
...
4. If 100 apps are present in the WEB APPS folder then for all the apps a separated ServletContext
   object will be created.

5. After deploying apps on server, container will look for Web.xml file in each and every folder of 
   WEB-INF.
...
10. If all this happens properly, then the server will be started properly.
...
12. When we press enter , request goes to the protocol.
...
19. In classes folder 100's of components are present. The request is coming for which componnent has
    to be identified using the URL patter or Resourcename(/web) which is coming from the client.
...
31. Once .class file is identified by the container then container will load the .class file
    batch code to the memory ( also called as Servlet class loading)
