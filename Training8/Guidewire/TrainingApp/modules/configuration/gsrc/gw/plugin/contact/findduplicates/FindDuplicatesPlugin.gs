...
uses com.guidewire.ab.system.logging.ABLoggerCategory
...
class FindDuplicatesPlugin implements gw.plugin.contact.IFindDuplicatesPlugin {
  ...
  ABContactSearchResult {
    ...
    finder.validateMandatoryFields()
    var queries = finder.makeQueries()
    ...
    if (resultHelper.Results != null) {
      ...
      for (i in 0..|resultHelper.Results.Count) {
        resultHelper.ResultsExact[i] = finder.isExactMatch(searchContact, resultHelper.Results[i])
      }
    }
    return resultHelper
  }
