...
class ClaimHealthCalculatorBatch extends BatchProcessBase {
  ...
  private function findNumberOfClaimsToProcess() : int {
    ...
    if (maxClaimResults < claimsWithNoMetricsCount) {
      PLLoggerCategory.SERVER_BATCHPROCESS.info(displaykey.Web.InternalTools.BatchProcess.ClaimHealthCalculator.ExceedsNumClaimResults(maxClaimResults, maxClaimResults, claimsWithNoMetricsCount))
  ...
  private function findClaims() : IQueryBeanResult<Claim> {
    ...
    query.compareNotIn("State", {ClaimState.TC_DRAFT, ClaimState.TC_ARCHIVED}.toArray())
  ...
  private function processClaim(claim : Claim) {
    try {
      ...
      PLLoggerCategory.SERVER_BATCHPROCESS.warn(displaykey.Web.InternalTools.BatchProcess.ClaimHealthCalculator.UpdateFailed(claim), e)
    }
  }
}
