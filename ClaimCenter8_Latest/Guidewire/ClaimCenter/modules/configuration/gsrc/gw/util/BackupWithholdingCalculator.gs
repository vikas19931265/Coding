...
class BackupWithholdingCalculator {
  ...
  public function computeDeductions(check : Check) : Deduction[] {
    ...
    if (withholdingRate != null) {
      ...
      Logger.logInfo(displaykey.Java.Financials.BackupWithholding.ComputeDeductions( withholding.ReservingAmount))
      return {withholding}
    }
    return new Deduction[0]
  }
  ...
  private static function getWithholdingRate( vendor : Contact) : BigDecimal {
      ...
      if (rate == null) {
        if (!(TaxStatus.TC_CONFIRMED == vendor.TaxStatus)) {
          ...
          Logger.logDebug(displaykey.Java.Financials.BackupWithholding.GetWithholdingRate.UnknownTaxStatus(vendor.DisplayName, standardWithholding))
        ...
        if (rate.IsZero) {
          ...
          Logger.logDebug(displaykey.Java.Financials.BackupWithholding.GetWithholdingRate.Rate(vendor.DisplayName, rate))
          return rate
        }
      }
  }
