...
uses gw.api.contact.CCContactMinimalTagsImpl
...
class ABContactSystemLinkPlugin implements ContactSystemLinkPlugin {
  ...
  static var _logger : Logger = CCLoggerCategory.CONTACT_SYSTEM_LINK_PLUGIN
  ...
  override function linkContact(contact: Contact, commit : boolean): LinkResult {
    ...
    match = new DuplicateContactMatchImpl(contact, null);
  ...
  override function link (contact: Contact, abuid: String) {
    ...
    if (contact.AddressBookUID != null && !abuid.equals(contact.AddressBookUID)) {
  ...
  private function unlink (contact : Contact) {
    ...
    for (cc in contact.AllContactContacts) {
      if (cc.SourceContact.AddressBookUID != null and !cc.SourceContact.AddressBookUID.equals(contact.AddressBookUID) and isPartOfClaim(cc.SourceContact.PublicID)) {
        ignoredABUID.add(cc.SourceContact.AddressBookUID)
      }

      if (cc.RelatedContact.AddressBookUID != null and !cc.RelatedContact.AddressBookUID.equals(contact.AddressBookUID) and isPartOfClaim(cc.RelatedContact.PublicID)) {
  ...
  private function isPartOfClaim(id : String) : boolean {
    var cc = Query.make(ClaimContact).select().where( \ c -> c.Contact.PublicID == id)
