...
class ISOReplyPlugin implements IISOReplyPlugin {
  ...
  override function handleISOReply(payload : ISOReplyPayload) {
    try {
      _callbackHandler.execute(\ -> { handleISOReplyInTransaction(payload.Parameters.get(ISOConstants.HTTP_REPLY_XML_PARAMETER_NAME)) })
    } catch( t : Throwable ) {
  ...
  override function handleKeyFieldUpdateSucceeded(messageId : int) {
    _callbackHandler.execute(\ ->{
      ...
      try {
        ...
        _logger.error(displaykey.Java.Logger.ISO.Message.Transport.KeyFieldUpdateAckFailed(messageId), e)
      }
    })
  }  
