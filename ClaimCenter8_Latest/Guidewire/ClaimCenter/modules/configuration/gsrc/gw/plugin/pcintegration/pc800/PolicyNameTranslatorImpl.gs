...
class PolicyNameTranslatorImpl extends NameTranslatorImpl {
  ...
  override function translateEntityName(entityName : String) : String {
    ...
    if (targetEntityType != null) {
      logResultsAtDebug(\ -> "translating from=" + entityName + " to=" + targetEntityName) // TODO ada make debug
    }
      ...
      if( targetEntityType != null && !targetEntityType.Array && !JavaTypes.LIST().isAssignableFrom(targetEntityType) )  {
        targetEntityName = targetEntityName.substring( 2 )
        logResultsAtDebug(\ -> "translating from=" + entityName + " to=" + targetEntityName) // TODO ada make debug
      }
      else {
        logResultsAtDebug(\ -> "translating from=" + entityName + " does not exist") // TODO ada make debug
        return null;
      }
    }
    else {
