...
class CatastropheSearchCriteria {
  ...
  /** Maximum number of bind variables supported by the database, where -1 means no known limit. */
  ...
  var _maxPointsForSearch : int as MaxPointsForSearch = MaxNumberOfBindVariables != -1 ? Math.min(MaxNumberOfBindVariables, MaxRows) : MaxRows

  // data for the table
  /** Data displayed in the claim table */
  var _catSearchClaimResults : ClaimSearchViewQuery as CatSearchClaimResults
  ...
  function updateMapAndTable() : boolean {  // return value is not meaningful
    ...
    if (changingCat) {
      ...
      if (totalPoints == 0 and (SelectedCatastropheKey != null)
        ...
        LocationUtil.addRequestScopedInfoMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.NoDataToShow)
      }
    ...
    if (MapVisible) {  // if the map is showing, generate the HTML (and the legend in case the language has changed)
      ...
      if (HeatMap.setupTemplate()) {
        ...
        LocationUtil.addRequestScopedErrorMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.CantFindTemplateClass(HeatMap.HeatMapServiceTemplateName))
      }
    }
  ...
  property get ClaimTableData() : ClaimSearchViewQuery {
  ...
  private function tableQuery() {
    ...
    if (HeatMap.hasClaims() and HeatMap.ClaimsDataSet.CountOfPointsSelected > MaxPointsForSearch) {
      if (changedSelection) {
        LocationUtil.addRequestScopedErrorMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.TooManyPoints(MaxPointsForSearch))
      }
    ...
    if (HeatMap.hasPolicyLocations() and HeatMap.PolicyLocationDataSet.CountOfPointsSelected > MaxPointsForSearch) {
      if (changedSelection) {
        LocationUtil.addRequestScopedErrorMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.TooManyPolicyLocationPoints(MaxPointsForSearch))
      }
    ...
    if (HeatMap.hasClaims() and showClaims) {
      ...
      if (changedSelection and claimCount != HeatMap.ClaimsDataSet.CountOfPointsSelected) {
        LocationUtil.addRequestScopedInfoMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.SecurityFiltered)
      }
    }
    ...
    if (claimCount == 0 and policyCount == 0) {
      TableVisible = false
      if (changedSelection) {
        LocationUtil.addRequestScopedInfoMessage(displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.NoResults)    
      }
    }
  }
  ...
  function performHeatMapPolicyLocationSearch( policyLocations : List<Key>) : List<PolicyLocationSearchResult> {
    var query1 = Query.make(PolicyLocationSummary)
                      .compareIn("ID", policyLocations as Object[])
                      .select()
                      .map( \ row -> {
    ...
    var query2 = Query.make(Claim)
                      ...
                      .compareIn("PolicyLocationSummary.ID", policyLocations as Object[])
                      .select(\ row -> {
                              return new PolicyLocationSearchResult(row.PolicyLocationSummaryJoin.PolicyLocationSummary.ID, row.ClaimNumber, row.ID)
                      })
                      .toList()
  ...
  static function dateFilterChoices() : List<DateRangeChoiceType> {
    if (dateFilterChoices == null) {
      ...
      dateFilterChoices = choices.where(\ choice -> (choice.Categories as ArrayList<TypeKey>)
  ...
  public static final var PL_WITH_CLAIM : PolicyGroupFilter = new PolicyGroupFilter() {
    ...
    override public function toString() : String {     
      return displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.PolicyLocationsWith
    }
  ...
  public static final var PL_WITHOUT_CLAIM : PolicyGroupFilter = new PolicyGroupFilter() {
    ...
    override public function toString() : String {
      return displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.PolicyLocationsWithout
    }
    override public function filter(hasClaim : boolean) : boolean {
      return not hasClaim
    }
