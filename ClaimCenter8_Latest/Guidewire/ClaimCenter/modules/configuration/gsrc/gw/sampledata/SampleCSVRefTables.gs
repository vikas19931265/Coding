...
class SampleCSVRefTables extends SampleDataBase {
  ...
  override function demoSampleData(bundle : Bundle) {
    ...
    // ICD codes are no longer stored as CSV, but they are still loaded from this class for historical continuity
    XMLSampleDataImporter.importFromFile(CCConfigResourceKeys.ICD10.File, bundle)
  }

  private function importCSVFile(filename : String) {
    var api = new ImportToolsImpl()
    var moduleRoot : java.io.File
    try {
      moduleRoot = ConfigAccess.getModuleRoot("cc")
    } catch (e : com.guidewire.modules.ConfigFileAccess.ModuleNotFoundException) {
      moduleRoot = ConfigAccess.getModuleRoot("configuration")
    }
    var file = new File(moduleRoot + "/config/referencedata/", filename)
