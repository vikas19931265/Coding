...
enhancement GWServiceRequestStateEnhancement : entity.ServiceRequest {
    ...
    return this.Progress == ServiceRequestProgress.TC_DRAFT 
      or (this.IsActive 
        and (this.QuoteStatus == ServiceRequestQuoteStatus.TC_WAITINGFORAPPROVAL
            or this.Invoices.hasMatch(\ i -> i.Status == ServiceRequestInvoiceStatus.TC_WAITINGFORAPPROVAL))
      )
      or (this.nextActionDefinition(stateHandler).ActionOwner != null and this.nextActionDefinition(stateHandler).ActionOwner == this.AssignedUser.Contact)
  }
  ...
  /**
   * Creates a new activity to the Service Request Owner indicating an operation was performed.
   */
  function createNewActivity(activityPattern: ServiceRequestActivityPattern) {
    createNewActivity(activityPattern, null, null, null)
  }
   ...
   */
  function createNewActivity(activityPattern: ServiceRequestActivityPattern, statement: ServiceRequestStatement) {
    createNewActivity(activityPattern, null, null, statement)
  }
  
  /**
   * Creates a new activity to the Service Request Owner indicating a change made to the service request.
   * This method is used to indicate when an specialist performed an operation
   */
  function createNewActivity(pattern: ServiceRequestActivityPattern, subject: String, description: String, statement: ServiceRequestStatement){
    var activity = new Activity()
    var activityPattern = ActivityPattern.finder.getActivityPatternByCode(pattern.PatternCode)
    activity.setActivityPattern(activityPattern, null, null)
    activity.Claim = this.Claim
