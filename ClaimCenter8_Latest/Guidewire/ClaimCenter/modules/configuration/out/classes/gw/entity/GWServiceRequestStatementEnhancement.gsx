...
enhancement GWServiceRequestStatementEnhancement : entity.ServiceRequestStatement {
  ...
  function linkDocumentForUI(document: Document, commitInNewBundle : boolean) {
    ...
    if (this.getMatchingServiceRequestStatementDocumentLink(document) != null) {
      ...
      var typeToDisplay = (this.Subtype == typekey.ServiceRequestStatement.TC_SERVICEREQUESTQUOTE) ? displaykey.Web.ServiceRequest.Quote.Lower : displaykey.Web.ServiceRequest.Invoice.Lower
      throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.Statement.DocumentDuplicate(document.Name, typeToDisplay))
    }
   ...
   */
  function attachDocument(docInfo: DocumentCreationInfo) {
  ...
  /**
   * Returns a list of pairs of document links and documents for this quote where
  ...
  private function verifyDocumentNameDoesNotExistOnStatement(document: Document){
    ...
    for (doc in this.StatementDocumentLinks) {
      if (doc.ServiceRequestDocumentLink.Document != null and doc.ServiceRequestDocumentLink.Document.Name == document.Name) {
        var typeToDisplay = (this.Subtype == typekey.ServiceRequestStatement.TC_SERVICEREQUESTQUOTE) ? displaykey.Web.ServiceRequest.Quote.Lower : displaykey.Web.ServiceRequest.Invoice.Lower
        throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.Statement.DocumentNameDuplicate(document.Name, typeToDisplay))
      }
    }
  }
  ...
  private function verifyDocumentNotSensitive(document: Document) {
    if (document.SecurityType == DocumentSecurityType.TC_SENSITIVE) {
      if (this.Subtype == typekey.ServiceRequestStatement.TC_SERVICEREQUESTQUOTE) {
        throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.Statement.SensitiveQuoteDocumentException) 
      }
      throw new gw.api.util.DisplayableException(displaykey.Web.ServiceRequest.Statement.SensitiveInvoiceDocumentException)
    }
  }
}