  ...
  override property get ApplicableDisplayValue() : String {
    if (not Metric.IsOpen and Metric.IntegerValue != null and not Metric.Skipped) {
      if (Metric.DaysDifferentFromExpected == 0) {
        return displaykey.Web.ServiceRequest.Metrics.ActualVsExpectedCompletionTime.ClosedOnTime
      }
      return ((Metric.DaysDifferentFromExpected < 0) 
                             ? displaykey.Web.ServiceRequest.Metrics.ActualVsExpectedCompletionTime.ClosedEarly(getFormattedTimeAsString(Metric.DaysDifferentFromExpected))
                             : displaykey.Web.ServiceRequest.Metrics.ActualVsExpectedCompletionTime.ClosedLate(getFormattedTimeAsString(Metric.DaysDifferentFromExpected)))
    }
    
    return (Metric.Skipped) ? getFormattedTimeAsString(Value) : displaykey.Web.ServiceRequest.Metrics.OpenDisplayValue(getFormattedTimeAsString(Value))
  }
