  ...
  override function validateCheckPayees () {
    ...
    for (checkPayee in payees) {
      ...
      if (payeeSet.contains(checkPayee.getPayee())) {
        throw new gw.api.util.DisplayableException(displaykey.Java.NewPayment.Error.DuplicatePayees)
      } else {
      ...
      if (ContactRole.TC_VENDOR==checkPayee.getPayeeType()) {
        if (vendorsFound == 1) {
          throw new gw.api.util.DisplayableException(displaykey.Java.NewPayment.Error.DuplicateVendor)
        } else {
          vendorsFound++
        }
      }
      ...
      if(ContactRole.TC_CLAIMANT==checkPayee.getPayeeType()) {
        if (not payeeHasRole(ContactRole.TC_CLAIMANT, checkPayee.getClaimContact())) {
          throw new gw.api.util.DisplayableException(displaykey.Java.NewPayment.Error.IllegalClaimantType)
        }
      }
      ...
      if(ContactRole.TC_INSURED==checkPayee.getPayeeType()) {
        if (not payeeHasRole(ContactRole.TC_INSURED, checkPayee.getClaimContact())) {
          throw new gw.api.util.DisplayableException(displaykey.Java.NewPayment.Error.IllegalInsuredType)
        }
      }
      ...
      if (not checkPayee.Check.Primary)
        if ( BackupWithholdingCalculator.isBackupWithholdingExpected(checkPayee) )
          throw new DisplayableException(displaykey.Java.NewPayment.Error.SecondaryVendorWithholding)
    }
  }
