...
class CatastropheHeatMap extends HeatMapGenerator {
  ...
  function showMultiplePointTooltip(clCount : int, plCount : int) : String {
    var tooltip : String
      if (clCount > 0 and plCount > 0) {
        if (clCount == 1) {
          tooltip = plCount == 1 ? displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultipleClaimsAndPolicyLocationsSS(clCount, plCount)
                                 : displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultipleClaimsAndPolicyLocationsSP(clCount, plCount)
        } else {
          tooltip = plCount == 1 ? displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultipleClaimsAndPolicyLocationsPS(clCount, plCount)
                                 : displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultipleClaimsAndPolicyLocationsPP(clCount, plCount)
        }
      } else if (clCount > 0) {
        tooltip = displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultipleClaims(clCount)
      } else {
        tooltip = displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.MultiplePolicyLocations(plCount)
      }
  ...
  function showClaimTooltip(claimPoint : CatastropheClaimLatLong) : String {
    ...
    if (claim == null || claim.Retired) {      
      tooltip = displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.ClaimRemoved
    } else {
      ...
      var names = claim.ClaimantNames.Count > 0
                  ? claim.ClaimantNames.join(displaykey.Web.Claim.Claimant.Seperator)
                  : claim.reporter
      ...
      tooltip = claim.ClaimantNames.Count > 0
                ? displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.ClaimTooltipClaimant(claimNumberPart, namesAddrPart, amount)
                : displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.ClaimTooltipReporter(claimNumberPart, namesAddrPart, amount)
    }
  ...
  function showPolicyLocationTooltip(policyLocationPoint : CatastrophePolicyLocationLatLong) : String {
    ...
    if (pls != null) { 
      ...
      tooltip = displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.PolicyLocationTooltip(
                maybeLink + "<br>",
                pls.InsuredName + "<br>" +
                pls.DisplayName +
                " <br>",
      ...
      tooltip = displaykey.JSP.ClaimSearch.Search.ClaimSearch.Catastrophe.PolicyLocationRemoved
    }
    return wrapToolTip(tooltip, position)  
  }

  /**
   * Returns true if CC is configured to download policy locations from Policy Center or another policy system.
